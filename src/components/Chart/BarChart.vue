...
const options = computed(() => {
  ...
  const { range } = props.setting
  const viewList = !range ? totalList : range === 'firstHalf' ? totalList.slice(0, 15) : totalList.slice(15)
  return {
    chart: {
      type: 'bar',
      stacked: true,
...
<template>
  <div class="chart">
    <VueApexCharts height="300" width="100%" :options="options" :series="series"></VueApexCharts>
  </div>
</template>
